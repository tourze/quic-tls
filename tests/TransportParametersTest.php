 ...
 * @internal
 */
#[CoversClass(TransportParameters::class)]
final class TransportParametersTest extends TestCase
{
    ...
    {
        $clientParams = new TransportParameters();
        $clientParams->setMaxIdleTimeout(30000);
        $clientParams->setMaxUdpPayloadSize(1500);
        $clientParams->setInitialMaxData(1000000);

        $serverParams = new TransportParameters();
        $serverParams->setMaxIdleTimeout(60000);
        $serverParams->setMaxUdpPayloadSize(1200);
        $serverParams->setInitialMaxData(500000);
        ...
        $this->assertEquals(30000, $negotiated->getMaxIdleTimeout());
        $this->assertEquals(1200, $negotiated->getMaxUdpPayloadSize());
        $this->assertEquals(500000, $negotiated->getInitialMaxData());
    }
    ...
    public function testValidate(): void
    {
        ...
        // 默认参数应该是有效的
        $this->assertTrue($params->validate());

        // 设置无效的 UDP 负载大小（太小的值会被自动调整，所以直接设置参数）
        $params->setParameter(TransportParameters::PARAM_MAX_UDP_PAYLOAD_SIZE, 100);
        $this->assertFalse($params->validate());

        // 设置无效的 UDP 负载大小（太大）
        $params->setMaxUdpPayloadSize(70000);
        $this->assertFalse($params->validate());

        // 重置为有效值
        $params->setMaxUdpPayloadSize(1500);
        $this->assertTrue($params->validate());
    }
}
