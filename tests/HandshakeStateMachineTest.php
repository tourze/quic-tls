 ...
 * @internal
 */
#[CoversClass(HandshakeStateMachine::class)]
final class HandshakeStateMachineTest extends TestCase
{
    ...
    private function processMultipleMessages(HandshakeStateMachine $stateMachine, string $data): string
    {
        ...
        while ($offset < strlen($data)) {
            ...
            $unpacked = unpack('N', "\x00" . substr($data, $offset + 1, 3));
            $messageLength = false !== $unpacked ? $unpacked[1] : 0;
    ...
    public function testProcessMessage(): void
    {
        ...
        // 创建有效的客户端握手消息
        $message = $this->createValidClientHello();

        // 处理消息应该返回服务端响应
        $response = $serverStateMachine->processMessage($message);
    ...
    public function testReset(): void
    {
        ...
        // 开始握手改变状态
        $clientStateMachine->startClientHandshake();
        $currentState = $clientStateMachine->getCurrentState();
        $this->assertNotEquals(HandshakeStateMachine::STATE_INITIAL, $currentState);

        // 重置状态机
        $clientStateMachine->reset();

        // 验证状态已重置
        $resetState = $clientStateMachine->getCurrentState();
        $this->assertEquals(HandshakeStateMachine::STATE_INITIAL, $resetState);
        $this->assertFalse($clientStateMachine->isComplete());
    }

    public function testStartClientHandshake(): void
    {
        $clientStateMachine = new HandshakeStateMachine(false, $this->transportParams, $this->certValidator);
        ...
        $this->assertNotEmpty($clientHello);
        $this->assertIsString($clientHello);

        // 验证状态已改变
        $state = $clientStateMachine->getCurrentState();
        $this->assertNotEquals(HandshakeStateMachine::STATE_INITIAL, $state);
    }
}
